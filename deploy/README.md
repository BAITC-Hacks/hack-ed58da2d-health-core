...
3. Скопировать `deploy/server.env.example` в `/opt/healthcore/api/.env`. Если используется Supabase, заменить только `DATABASE_URL` на строку `postgresql+psycopg://...` с `sslmode=require`.
4. Из каталога `/opt/healthcore/api` выполнить `.venv/bin/python -m backend.cli init-db`, затем `import-csv 1 data/turbine-1.csv`, `import-csv 2 data/turbine-2.csv`, `train`.
...
7. Проверить на сервере `curl http://127.0.0.1:8000/health`. Для локального доступа использовать SSH-туннель `ssh -L 8000:127.0.0.1:8000 root@77.37.65.54` и открыть `http://127.0.0.1:8000/docs`.
