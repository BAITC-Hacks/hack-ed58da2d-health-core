...
class MeasurementInput(BaseModel):
    turbine_id: int = Field(ge=1, le=2)
...
def measurements(turbine_id: int = Query(ge=1, le=2), limit: int = Query(default=100, ge=1, le=1000),
...
def forecast(issue_date: date, session: Session = Depends(get_session)) -> dict:
    try:
